...
async function main() {
  ...
  const trustFactory = await hre.reef.getContractAt("TrustFactory", trustFactoryAddress, signer);

  //  --- Properties ---
  // The read/write tier
  const TierFactory = await hre.reef.getContractFactory("ReadWriteTier", signer);
  ...
  const ReserveTokenFactory = await hre.reef.getContractFactory("ReserveToken", signer);
